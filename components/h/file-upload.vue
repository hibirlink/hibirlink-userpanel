...
const { value, errorMessage: fileError } = useField(props.name);
...
const drop_and_click = async (files: any) => {
  ...
  await Promise.all(
    [...files].map((f) => {
      if (f && sizesValidation(formatBytes(f.size))) return readFile(f);
    })
  );
...
<template>
  <h-drag-and-drop
    :class="props.class"
    class="min-h-[13rem] relative py-2 border-dashed border-[3px] rounded-md border-gray-light hover:border-green300"
  >
    <template #default="{ drag, hover }">
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <div
                class="absolute opacity-90 z-20 flex justify-between items-end -bottom-1 px-2 py-1 left-0 w-full text-green1100 bg-gradient-to-tr from-green100 to-transparent invisible group-hover:visible"
              >
            ...
            <div
              v-if="limit > 1"
              class="bg-white rounded-[100%] flex items-center cursor-pointer justify-center absolute w-6 h-6 top-0 right-1"
        ...
        <label
          :for="id"
          class="flex flex-col w-full gap-0 items-center justify-center cursor-pointer"
        >
          ...
          <div
            v-if="
              typeof file !== 'string' && (limit !== 1 || file.length !== 1)
            "
            class="flex flex-col w-full gap-0 items-center justify-center cursor-pointer"
          >
            <CloudArrowUpIcon class="text-green300 w-10 h-10" />
            <span class="text-sm text-green300"
              >Drag & drop or Click this
            </span>
            <span class="text-sm w-full text-center text-gray-light"
              >Image Size Should be {{ props.size }} or less! <br />
              Upload Up to {{ props.limit }} Files</span
            >
          </div>
